...
import com.lymin.ptahchnotmanager.adapter.LotteryListDetailAdapter
...
class LotteryDetailActivity : AppCompatActivity() {
    ...
    val adapter = LotteryListDetailAdapter(this@LotteryDetailActivity,listLottery)
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<ImageButton>(R.id.btn_add).setOnClickListener {
            startActivity(Intent(this@LotteryDetailActivity,AddDataActivity::class.java))
        }

        val chnotDetailItem = intent.getParcelableExtra<ChnotModel>("chnotDetailItem")

        if (chnotDetailItem!=null){
            initData(chnotDetailItem)
        }

    }

    private fun initData(chnotDetailItem: ChnotModel) {
        listLottery.add(chnotDetailItem)
