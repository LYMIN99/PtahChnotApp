...
class LotteryTN3Activity : AppCompatActivity() {
    ...
    private fun initData() {
        binding.tvTime.text = "4:15"
        tvDate = findViewById(R.id.tv_date)
        val ysd = Utils.getYesterday()
        tvDate.text = ysd

        tvDate.setOnClickListener {
            DialogPickDate(this@LotteryTN3Activity,object : DialogPickDate.OnCallBack{
    ...
    private fun saveData() {
        ...
        FirebaseHelper().saveLotteryTn3ToFirestore(data,object : FirebaseHelper.OnUploadCallBack{
            override fun onSuccess() {
                Toast.makeText(this@LotteryTN3Activity,"Upload Success", Toast.LENGTH_SHORT).show()
                finish()
            }

            override fun onFailed() {
                Toast.makeText(this@LotteryTN3Activity,"Upload Error", Toast.LENGTH_SHORT).show()
            }
        })
    }
}